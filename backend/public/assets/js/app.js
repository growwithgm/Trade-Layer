...
// Returns ?shop=xxx&host=xxx from the current URL, or '' if neither present.
function getNavQS() {
...
// Navigate to a page, preserving shop/host query params.
// Inside Shopify's iframe, open() with '_top' navigates the outer frame —
// this is the standard Shopify embedded-app navigation pattern.
function navigateTo(page) {
  var qs = getNavQS();
  var url = window.location.origin + '/' + page + qs;
  // Embedded in Shopify admin iframe — must break out of frame.
...
function renderSidebar(activeFile) {
  ...
  function item(page, sub) {
    ...
    return '<a href="#" class="' + cls + active + '" onclick="event.preventDefault();navigateTo(\'' + page.file + '\')">'
  ...
  nav.innerHTML =
    ...
    '<a href="#" class="nav-item" onclick="event.preventDefault();navigateTo(\'index.html\')"><i data-lucide="arrow-left-right" class="w-4 h-4"></i><span>Return to Shopify</span></a>';
}
    ...
    if (type === 'sales') {
      new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{label:'Sales', data:[4200,6200,5800,8900,7600,11200,12800],
            borderColor:'#5c59f2', backgroundColor:'rgba(92,89,242,.08)',
            pointBackgroundColor:'#5c59f2', fill:true, tension:.35, borderWidth:2}]
        },
        options: {responsive:true, maintainAspectRatio:false,
          plugins:{legend:{display:false}},
          scales:{y:{grid:{color:'#f1f5f9'}, ticks:{color:'#94a3b8'}},
    ...
    if (type === 'quotes') {
      new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{data:[8,11,7,14,13,6,4], backgroundColor:'#5c59f2',
            borderRadius:8, maxBarThickness:28}]
        },
        options: {responsive:true, maintainAspectRatio:false,
          plugins:{legend:{display:false}},
          scales:{y:{grid:{color:'#f1f5f9'}, ticks:{color:'#94a3b8'}},
